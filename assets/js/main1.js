function directus(collection) {
  return (
    `http://localhost/mall.directus/public/mall/items/` +
    collection +
    `?access_token=OTlKH3cQGQovo89kr1s6auzW`
  );
}
...
function displayCart() {
  ...
  for (var i in cartArray) {
    ...
    $("#carto").val(JSON.stringify(cartArray));
...
async function refreshCoupon(couponCode) {
  ...
  $("#finalprice").html(
    `<input type='hidden' class='form-control' name='price' placeholder='' value=${disc}>`
  );
}
